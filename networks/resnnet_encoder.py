...
def resnet_multiimage_input(num_layers, pretrained=False, num_input_images=1):
    ...
    if pretrained:
        ...
        loaded = torch.load('./models/resnet34.pth')
...
class ResnetEncoder(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, num_input_images=1):
        ...
        if num_input_images > 1:
            ...
            if pretrained:
                self.encoder.load_state_dict(torch.load('./pretrain_models/resnet34.pth'))
...
if __name__ == "__main__":
    model = ResnetEncoder(num_layers=18, pretrained=False)
    x = torch.randn((6, 3, 352, 640))
    fs = model(x)
    for f in fs:
        print(f.shape)
