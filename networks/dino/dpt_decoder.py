...
from networks.transformer import CVT
...
class DPT_decoder(nn.Module):
    def __init__(self, encoder='vitl', nclass=1, features=256, out_channels=[256, 512, 1024, 1024], use_bn=True, use_clstoken=False, skip=True):
        ...
        for i in range(len(out_channels)):
            self.cross[i] = CVT(input_channel=out_channels[i], downsample_ratio=down_ratio[i], iter_num=4)
