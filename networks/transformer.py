...
class CVT(nn.Module):
    def __init__(self,input_channel,downsample_ratio=2, iter_num=4, num_heads=8):
        super().__init__()
        self.embed_dim = input_channel
        self.iter_num = iter_num
        self.self_block = []
        for i in range(iter_num):
            self.self_block.append(Self_Block(dim=self.embed_dim, num_heads=num_heads))
...
class SimpleCVT(nn.Module):
    def __init__(self, input_channel, iter_num=4, num_heads=8):
        super().__init__()
        self.embed_dim = input_channel
    ...
    def forward(self, x):
        ...
        for i in range(self.iter_num):
            ...
            self.pos_embed = self.postional_embed(x[:,0,:,:,:]).unsqueeze(1).repeat(1,N,1,1,1)
            
            x = x.permute(0,1,3,4,2).reshape(B,-1,C)
            
            x_pos = self.pos_embed.permute(0,1,3,4,2).reshape(B,-1,C)
            
            x = self.self_block[i](x, x_pos).reshape(B,N,H,W,C).permute(0,1,4,2,3)
...
class SimpleAdj(nn.Module):
    def __init__(self, input_channel=3, iter_num=4, num_heads=8):
        super().__init__()
        self.embed_dim = input_channel
        ...
        for i in range(iter_num):
            self.self_block.append(Self_Block(dim=self.embed_dim, num_heads=num_heads))
            self.cross_block.append(Cross_Block(dim=self.embed_dim, num_heads=num_heads))
    
        self.decoder = nn.ModuleList(list(self.self_block)+list(self.cross_block))
    ...
    def forward(self, x):
        ...
        B, N, C, H, W = x.shape
        for i in range(self.iter_num):
            self.pos_embed = self.postional_embed(x[:,0,:,:,:])   # B x C x H x W
            x = x.view(-1, C, H, W)     # B*N x C x H x W
            y = torch.zeros_like(x,device=x.device)
            for j in range(N):
                # Self-Attention
                view_features = x[j::N]    # B x C x H x W
                view_features = view_features.view(B, C, H*W).transpose(1, 2)    # B x (H*W) x C
                view_features_pos = self.pos_embed.permute(0,2,3,1).reshape(B,-1,C)
                self_att_output = self.self_block[i](view_features, view_features_pos).reshape(B, H, W, C)
             
                adj_output = [self_att_output]
                # Adjacent-Attention
                for k in [-1, 1]:
                  adjacent_view_features = x[((j+k) % N)::N]
                  adjacent_view_features = adjacent_view_features.view(B, C, H*W).transpose(1, 2)    # B x (H*W) x C
                  adj_output.append(self.cross_block[i](view_features, adjacent_view_features, view_features_pos).reshape(B, H, W, C)) # B x H x W x C
                new_output = sum(adj_output)
                # Combine self-attention and adjacent-attention results
                y[j::N] = new_output.permute(0,3,1,2)
...
class Cross_Block(nn.Module):
    ...
    def forward(self, cur, adj, pos):
        x = cur + self.drop_path(self.attn(self.norm_self(cur), self.norm_adj(adj), pos))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x # [B, q_len, mlp_dim]
...
if __name__ == '__main__':
  model = SimpleAdj(input_channel=8*8)
  B, N, C, H, W = 4, 6, 8*8, 24, 26
