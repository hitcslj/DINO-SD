...
from .mono_dataset_dino import MonoDataset
...
class NuscDataset(MonoDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.data_path = 'data/nuscenes/raw_data' # ../data/nuscenes/raw_data
    ...
    def get_gt(self,lidar_sample,cam_sample):
        def generate_image_from_points(points, features, imsize):
            ...
            return projection
        points, depth, _ = self.nusc_explorer.map_pointcloud_to_image(pointsensor_token=lidar_sample["token"],camera_token=cam_sample["token"])
        lidar_proj = generate_image_from_points(points[:2], depth, (self.opt.height_ori, self.opt.width_ori))
        return torch.tensor(np.expand_dims(lidar_proj, axis=0))
    ...
    def get_info(self, inputs, index_temporal, do_flip):
        ...
        for index_spatial in range(6):
            ...
            color = self.loader(os.path.join(self.data_path, cam_sample['filename']),k=self.scale)
            ...
            inputs[("gt")].append(self.get_gt(lidar_sample, cam_sample))
            if not self.is_train:
                inputs[("depth")].append(np.squeeze(self.get_gt(lidar_sample, cam_sample), 0))
            ...
            if self.is_train:
                ...
                for idx, i in enumerate(self.frame_idxs[1:]):
                    ...
                    color = self.loader(os.path.join(self.data_path, cam_sample_i['filename']),k=self.scale)
                    if do_flip:
                        color = color.transpose(pil.FLIP_LEFT_RIGHT)
