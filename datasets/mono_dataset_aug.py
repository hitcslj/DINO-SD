...
# def resize_tensor_img(img,k):
#     orginal_shape=img.shape[1:]
#     return F.interpolate(img.unsqueeze(0), size=(int(orginal_shape[0]/k), int(orginal_shape[1]/k)), mode='bilinear', align_corners=False).squeeze(0)
...
class MonoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 把color 0，-1，1取出来，然后做两个增强，合并到inputs里面
        # inputs[("color_aug_1", 0, scale)]
