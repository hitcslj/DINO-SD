...
from denoise import histogram_equalization_rgb_pil,universal_denoise_pil,needs_denoising
...
class CorruptionDataset(MonoDataset):
    ...
    def get_info(self, inputs, index_temporal, do_flip):
        ...
        for index_spatial in range(6):
            ...
            #denoise
            if "universal_denoise" in self.opt.denoise:
                if needs_denoising(color):
                    color=universal_denoise_pil(color)
                    
            if "equalize" in self.opt.denoise:
                color=histogram_equalization_rgb_pil(color)
            
            inputs['width_ori'].append(color.size[0])
            inputs['height_ori'].append(color.size[1])
            inputs['filenames'].append(cam_sample['filename'].split('/')[-1])
