...
import datasets
import networks
import networks.dino as dino
from IPython import embed
from datasets.nusc_dataset_dino import NuscDataset
...
def upScale(depth_map):
    return F.interpolate(depth_map, (900, 1600), mode='bilinear', align_corners=True)
...
class SILogLoss(nn.Module):  # Main loss function used in AdaBins paper
    ...
    def forward(self, input, target, interpolate=True):
        ...
        mask = torch.logical_and(0.1<target,target<80) 
        input = input[mask]
        target = target[mask]
        g = torch.log(input) - torch.log(target)

        # Dg = torch.var(g) + 0.15 * torch.pow(torch.mean(g), 2)
        Dg = torch.mean(torch.pow(g, 2.0)) - 0.5 * torch.pow(torch.mean(g), 2)
...
class Runer:
    def __init__(self, options):
        ...
        self.encoder_parameters_to_train = []
        self.decoder_parameters_to_train = []
        ...
        self.models["encoder"] = dino.DINO_Encoder(encoder=self.opt.encoder, patch_size=14)
        ...
        self.encoder_parameters_to_train += list(self.models["encoder"].parameters())

        self.models["depth"] = dino.DPT_decoder_ADJ(encoder=self.opt.encoder, out_channels=[256, 512, 1024, 1024])
        ...
        self.decoder_parameters_to_train += list(self.models["depth"].parameters())
        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                ...
                self.pose_parameters_to_train += list(self.models["pose_encoder"].parameters())
        ...
        if self.opt.predictive_mask:
            ...
            self.pose_parameters_to_train += list(self.models["predictive_mask"].parameters())
        ...
        self.model_optimizer = optim.AdamW([
                                        {'params':self.pose_parameters_to_train, 'lr':self.opt.learning_rate_pose},
                                        {'params':self.encoder_parameters_to_train, 'lr':self.opt.learning_rate_encoder},
                                        {'params':self.decoder_parameters_to_train, 'lr':self.opt.learning_rate_decoder}])
        ...
        self.model_lr_scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(
            self.model_optimizer, T_0= epochs // 2, T_mult = 2
        )
    ...
    def val(self):
        ...
        with torch.no_grad():
            ...
            for idx, data in enumerate(loader):
                if self.local_rank == 0:
                    print(f"\rEvaluating minibatch {idx + 1}/{len(loader)}", end="")
                input_color = data[("color", 0, 0)].cuda()
                gt_depths = data["depth"].cpu().numpy()
                
                camera_ids = data["id"]
                features, ph, pw = self.models["encoder"](input_color)
                output = self.models["depth"](features, ph, pw)
                ...
                features_flip, ph, pw = self.models["encoder"](input_color_flip)
                output_flip = self.models["depth"](features_flip, ph, pw)
                ...
                for i in range(pred_disps.shape[0]):
                    ...
                    gt_height, gt_width = gt_depth.shape[:2] # 900, 1600
    
                    pred_disp = pred_disps[i]
                    pred_depth = 1 / pred_disp                   
                    pred_depth = cv2.resize(pred_depth, (gt_width, gt_height)) # 900, 1600
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            if early_phase or late_phase:
                self.log_time(batch_idx, duration, losses["loss"].cpu().data, losses["augmix_loss"].cpu().data, losses['sup_loss'].cpu().data)
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            features, ph, pw = self.models["encoder"](inputs["color", 0, 0])
            features_aug_1, _, _ = self.models["encoder"](inputs["color_aug_1", 0, 0])
            features_aug_2, _, _ = self.models["encoder"](inputs["color_aug_2", 0, 0])
            outputs = self.models["depth"](features, ph, pw)
            outputs_aug_1 = self.models["depth"](features_aug_1, ph, pw)
            outputs_aug_2 = self.models["depth"](features_aug_2, ph, pw)
            for i in range(self.num_scales):
                outputs[("disp_aug_1", i)] = outputs_aug_1[("disp",i)]
                outputs[("disp_aug_2", i)] = outputs_aug_2[("disp",i)]

        if self.opt.predictive_mask:
            outputs["predictive_mask"] = self.models["predictive_mask"](features)

        if self.use_pose_net and self.use_photometric:
        ...
        if self.num_pose_frames == 2:
            ...
            if self.opt.pose_model_type in ["separate_resnet", "posecnn"]:
                pose_inputs = torch.cat(
                    [inputs[("colo", i, 0)] for i in self.opt.frame_ids if i != "s"], 1)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            # aug_loss
            def outputs2depth_norm(output):
                B, _, H, W = output.shape
                disp=output.reshape(output.shape[0],-1)
                _, depth=disp_to_depth(disp,min_depth=self.opt.min_depth,max_depth=self.opt.max_depth)
                depth_norm=F.softmax(depth,dim=1)
                return depth.reshape(B, -1, H, W), depth_norm
            
            

            depth_clean, depth_norm_orig = outputs2depth_norm(outputs[("disp",scale)])
            depth_aug_1, depth_norm_aug_1 = outputs2depth_norm(outputs[("disp_aug_1",scale)])
            depth_aug_2, depth_norm_aug_2 = outputs2depth_norm(outputs[("disp_aug_2",scale)])
            mixture_depth=torch.clamp((depth_norm_orig+depth_norm_aug_1+depth_norm_aug_2)/3.,1e-7, 1).log()    
            aug_loss = aug_lambda * (
                F.kl_div(mixture_depth, depth_norm_orig, reduction='batchmean') +
                F.kl_div(mixture_depth, depth_norm_aug_1, reduction='batchmean') +
                F.kl_div(mixture_depth, depth_norm_aug_2, reduction='batchmean')) / 3.
            mix_depth = (depth_clean + depth_aug_1 + depth_aug_2) / 3.0
            
            losses["augmix_loss"] += (aug_loss/(2**scale)) / self.num_scales
            loss +=  aug_loss
            # sup_loss
            silogloss = SILogLoss()
            # sup_loss = silogloss(disp_to_depth(outputs[("disp",scale)],self.opt.min_depth,self.opt.max_depth)[1], inputs[("gt")])
            sup_loss = silogloss(mix_depth, inputs[("gt")])
            
            losses["sup_loss"] += (sup_loss/(2**scale)) / self.num_scales
            loss += sup_loss
    ...
    def log_time(self, batch_idx, duration, loss, augloss = 0.0,suploss=0.0):
        ...
        if self.local_rank == 0:
            ...
            print_string = "epoch {:>3} | batch {:>6} | examples/s: {:5.1f}" + \
                " | loss: {:.5f} | aug_loss: {:.5f} | sup_loss: {:.5f} | time elapsed: {} | time left: {}"
            
            self.log_print(print_string.format(self.epoch, batch_idx, samples_per_sec, loss, augloss,suploss,sec_to_hm_str(time_sofar), sec_to_hm_str(training_time_left)))
    ...
    def log_print(self, str):
        print(str)
        with open(os.path.join(self.log_path, 'log_clean.txt'), 'a') as f:
            f.writelines(str + '\n')
